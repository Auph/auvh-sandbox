...
export function buildEnvVars(env: MoltbotEnv): Record<string, string> {
  ...
  // Map GATEWAY_TOKEN (or legacy MOLTBOT_GATEWAY_TOKEN) to OPENCLAW_GATEWAY_TOKEN for container
  const gatewayToken = env.GATEWAY_TOKEN ?? env.MOLTBOT_GATEWAY_TOKEN;
  ...
  if (env.CF_ACCOUNT_ID) envVars.CF_ACCOUNT_ID = env.CF_ACCOUNT_ID.trim();
  if (env.CDP_SECRET) envVars.CDP_SECRET = env.CDP_SECRET;
  if (env.WORKER_URL) envVars.WORKER_URL = env.WORKER_URL;

  // R2 persistence credentials (used by rclone in start-openclaw.sh)
  if (env.R2_ACCESS_KEY_ID) envVars.R2_ACCESS_KEY_ID = env.R2_ACCESS_KEY_ID.trim();
  if (env.R2_SECRET_ACCESS_KEY) envVars.R2_SECRET_ACCESS_KEY = env.R2_SECRET_ACCESS_KEY.trim();
  if (env.R2_BUCKET_NAME?.trim()) envVars.R2_BUCKET_NAME = env.R2_BUCKET_NAME.trim();
