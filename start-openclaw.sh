...
# Fail fast on restore - avoid hanging 20+ min when R2 creds are wrong
RCLONE_RESTORE_FLAGS="$RCLONE_FLAGS --timeout=30s --contimeout=10s --low-level-retries=1 --retries=1"
...
if r2_configured; then
    ...
    # Restore skills
    REMOTE_SK_COUNT=$(rclone ls "r2:${R2_BUCKET}/skills/" $RCLONE_RESTORE_FLAGS 2>/dev/null | wc -l)
    if [ "$REMOTE_SK_COUNT" -gt 0 ]; then
        echo "Restoring skills from R2 ($REMOTE_SK_COUNT files)..."
        mkdir -p "$SKILLS_DIR"
        rclone copy "r2:${R2_BUCKET}/skills/" "$SKILLS_DIR/" $RCLONE_RESTORE_FLAGS -v 2>&1 || echo "WARNING: skills restore failed with exit code $?"
