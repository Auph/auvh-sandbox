...
**Free, open-source personal AI on the edge.** Run [OpenClaw](https://github.com/openclaw/openclaw) (Claude-powered assistant) in your own Cloudflare account. You own your data and hosting—this is a guided setup tool, not a managed service.

![ClawWorker architecture](./assets/logo.png)

[![Deploy to Cloudflare](https://deploy.workers.cloudflare.com/button)](https://deploy.workers.cloudflare.com/?url=https://github.com/Auph/clawworker)

## Upgrading from an older fork?
...
- **R2 bucket:** New installs use `clawworker-data`. If you have data in `moltbot-data` or `moltworker-data` (from Moltworker): (1) set `R2_BUCKET_NAME` secret; (2) create `.r2-bucket` with the bucket name (e.g. `echo moltworker-data > .r2-bucket`) — this file is gitignored so it won't revert on pull.

## Requirements

- [Cloudflare account](https://dash.cloudflare.com/sign-up) — Workers Paid plan ($5 USD/month) for Sandbox containers
...
![ClawWorker architecture](./assets/architecture.png)
...
Run the setup wizard for a guided web interface:

```bash
npm run setup:wizard
```

Opens a local server where you can enter configurations step by step and deploy to Cloudflare. Deploy the wizard to Cloudflare Pages:

**Option A: Direct deploy (no re-entering secrets)**
```bash
npm run setup:wizard
```
Then complete the wizard and click **Deploy**. When run locally, it deploys directly via wrangler—no Cloudflare form. Requires `npx wrangler login` or `CLOUDFLARE_API_TOKEN`.

**Option B: Direct upload (CLI)**
```bash
npm run setup:wizard:deploy
```
Requires `CLOUDFLARE_API_TOKEN` in your environment.

**Option C: Connect to Git (Cloudflare Dashboard)** — for hosting the wizard only
...
2. Prompt for your Anthropic API key (or Cloudflare AI Gateway credentials)
3. Generate a gateway token
4. Deploy to Cloudflare and configure secrets

After setup, open the URLs shown (add your worker URL from the deploy output):

- **Control UI (chat):** `https://YOUR-WORKER.workers.dev/?token=YOUR_GATEWAY_TOKEN`
- **Admin UI (device pairing):** `https://YOUR-WORKER.workers.dev/_admin/?token=YOUR_GATEWAY_TOKEN`

**Bootstrap mode:** Without Cloudflare Access, you can use the gateway token to access both the Control UI and Admin UI. Visit `/_admin/?token=YOUR_TOKEN` to pair devices. For production, [set up Cloudflare Access](#setting-up-the-admin-ui) for stronger security.
...
Then open `https://YOUR-WORKER.workers.dev/?token=YOUR_TOKEN` and `/_admin/?token=YOUR_TOKEN` for device pairing.

### Automatic deployment (GitHub Actions)

Pushes to `main` trigger a deploy via GitHub Actions. Add these [repository secrets](https://docs.github.com/en/actions/security-guides/encrypted-secrets):

| Secret | Description |
|--------|-------------|
| `CLOUDFLARE_API_TOKEN` | [Create](https://dash.cloudflare.com/profile/api-tokens) an API token with "Edit Cloudflare Workers" template |
| `CLOUDFLARE_ACCOUNT_ID` | Your [Cloudflare Account ID](https://dash.cloudflare.com/?to=/:account/workers) |

Worker secrets (ANTHROPIC_API_KEY, R2 credentials, etc.) are stored in Cloudflare and persist across deploys — they do not need to be in GitHub.

### Required: R2 storage

R2 is **required** for memory persistence. Your conversations, paired devices, and workspace are synced to R2 so they survive container restarts. See [Persistent Storage (R2)](#persistent-storage-r2) for setup.

### Optional

- [Cloudflare Access](#setting-up-the-admin-ui) — Recommended for production (replaces bootstrap token auth)

## Setting Up the Admin UI

**Bootstrap mode (default):** With just `GATEWAY_TOKEN` set, visit `/_admin/?token=YOUR_TOKEN` to access the admin UI. No Cloudflare Access required.

**Production (recommended):** For multi-user access and stronger security:
...
2. Select your Worker (e.g., `clawworker`)
...
4. Set the application domain to your Worker URL (e.g., `clawworker.your-subdomain.workers.dev`)
...
**Bootstrap mode:** When Cloudflare Access is not configured, the gateway token also unlocks the Admin UI at `/_admin/?token=YOUR_TOKEN`. This lets you pair devices immediately after deploy without setting up Access first. For production, add Cloudflare Access for stronger security.
...
R2 storage is **required**. ClawWorker syncs configs, paired devices, conversation history, and workspace to R2 so your memory persists across container restarts.
...
4. Select the `clawworker-data` bucket (created automatically on first deploy)
...
R2 credentials are required at deploy time. Without them, the worker will not start.
...
| `R2_ACCESS_KEY_ID` | Yes | R2 access key (required for memory persistence) |
| `R2_SECRET_ACCESS_KEY` | Yes | R2 secret key (required for memory persistence) |
| `CF_ACCOUNT_ID` | Yes | Cloudflare account ID (required for R2 storage) |
